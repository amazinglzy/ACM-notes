...
# 优化KMP
## Description
$g[i][j]$ 数组表示子串当前匹配状态是 $[1..i]$ 时输入字符 $j$ 时的匹配状态。
$c[i][j]$ 数组表示子串当前匹配状态是 $[1..i]$ 时输入字符 $j$ 时能否完全匹配子串。
## Template
```cpp
int nxt[N];
int g[N][M];
...
void GetExtend(string S, string T, int extend[], int next[]) {
    int a; int p; //记录匹配成功的字符的最远位置p，及起始位置a
    int s_len = S.size();
    int t_len = T.size();
    ...
    {
        if (j < 0 || i + next[i - a]>= p)  /*i大于p（其实j最小只可以到-1 j<0 的写法方便读者理解程序），*/
